...
def photo(img, path, x, y, w, h, placeholder_kind=None, placeholder_label=None,
    ...
    if markers and isinstance(markers, dict):
        try:
            ...
            try:
                fnt = ImageFont.truetype(KR_FONT or LATIN_R, small_size)
            except Exception:
                try: fnt = ImageFont.load_default()
                except Exception: fnt = None
            
            def safe_text_size(text):
                """안전한 텍스트 크기 추정 (textbbox 없는 환경 대비)"""
            ...
            def draw_label(text, lx, ly):
                ...
                try:
                    ...
                    if fnt is not None:
                        nd.text((lx, ly), text, fill="white", font=fnt)
                    else:
            ...
            try:
                ...
                if lines and fnt is not None:
                    ...
                    for text, (tw_, th_) in zip(lines, sizes):
                        try:
                            d.text((ox+pad_x, cy), text, fill="white", font=fnt)
                        except Exception: pass
                        cy += th_ + gap
            except Exception: pass
